...
/** \file COM_base.C
 * Contains the base implementation of the COM.
...
COM_base *COM_base::com_base=NULL;


/// Set the COM pointer to the given object.
/// It was introduced to support processes.
void COM_base::set_com(COM_base *r)  { com_base = r; }
...
COM_base::COM_base( int *argc, char ***argv) throw( COM_exception, int)
    ...
    _mpi_initialized(false), _errorcode(0), _exception_on(true), _profile_on(0)
{
  ...
  const char *comhome = std::getenv( "COM_HOME");
  ...
  if ( _verbose) {
    std::cerr << "COM Version 1.1, 11/01/2013" << std::endl;
    std::cerr << "COM: Started with verbose level " << _verbose << std::endl;
#ifndef DUMMY_MPI
    if ( !COMMPI_Initialized())
      std::cerr << "COM: MPI not initialized. Running in serial mode\n";
#else
    std::cerr << "COM: Running in serial mode with DUMMY_MPI\n";
#endif
  }
}

COM_base::~COM_base() {
  // If MPI was initialized by COM, then call MPI_Finalize.
  if (_mpi_initialized) MPI_Finalize();
}

void COM_base::init( int *argc, char ***argv ) throw( int) {
  extern void printStackBacktrace();

  COM::COM_base *ptr = get_com();
  if ( ptr) throw( COM_ERR_WAS_INITIALIZED);

  ptr = new COM::COM_base( argc, argv);
  
  set_com( ptr);
}

void COM_base::finalize() throw( int) {
  COM::COM_base *ptr = get_com();
...
void COM_base::abort( int ierr) {
  if ( COMMPI_Initialized() ) MPI_Abort( MPI_COMM_WORLD, ierr);
  else exit( ierr); 
}

void
COM_base::load_module( const std::string &lname, 
...
void
COM_base::unload_module( const std::string &lname,
...
void COM_base::new_window( const std::string &name, MPI_Comm comm) throw(int) 
{
...
window_init_done( const std::string &wname, bool panechanged) throw(int) 
{
  try {
    ...
    ex.msg = append_frame( ex.msg, COM_base::window_init_done);

    std::string msg = std::string("When completing the init of window ")+wname;
    proc_exception( ex, msg);
  }
  if ( _verb1>1) 
    std::cerr << "COM: Done creating window \"" << wname 
	      << '"' << std::endl;
}

void COM_base::delete_window( const std::string &name) throw(int) 
{
  try {
    if ( _verb1>1)
      std::cerr << "COM: Deleting window \"" << name << '"' << std::endl;
    ...
    ex.msg = append_frame( ex.msg, COM_base::delete_window);
    std::string s;
...
void COM_base::delete_pane( const std::string &wname,
			       const int pane_id) throw(int) 
{
  try {
    if ( _verb1>1)
      std::cerr << "COM: Delete pane " << pane_id << " of window "
		<< std::endl;
    ...
    ex.msg = append_frame( ex.msg, COM_base::delete_pane);
    char buf[10];
...
// Register a new dataitem with given name
void COM_base::
new_dataitem( const std::string &wa, 
	       const char loc,
	       const int type, 
	       int size,
	       const std::string &unit) throw(int) 
{
  try {
    ...
    // Invoke Window::new_dataitem.
    std::string wname, aname;
    split_name( wa, wname, aname);
    
    get_window( wname).new_dataitem( aname, loc, type, size, unit);
    _errorcode = 0;
  }
  catch ( COM_exception ex) {
    ex.msg = append_frame( ex.msg, COM_base::new_dataitem);
    std::string s;
    s = s + "When processing dataitem " + wa;
    proc_exception( ex, s);
  }
}
...
// Register a new dataitem with given name
void COM_base::
delete_dataitem( const std::string &wa) throw(int) 
{
  try {
    if ( _verb1>1)
      std::cerr << "COM: delete dataitem \"" << wa << std::endl;

    // Invoke Window::new_dataitem.
    std::string wname, aname;
    split_name( wa, wname, aname);
    
    get_window( wname).delete_dataitem( aname);
    _errorcode = 0;
  }
  catch ( COM_exception ex) {
    ex.msg = append_frame( ex.msg, COM_base::delete_dataitem);
    std::string s;
    s = s + "When processing dataitem " + wa;
    proc_exception( ex, s);
  }
...
void COM_base::set_size( const std::string &wa, int pid,
			    int nitems, int ng) throw(int)
{
  try {
    if ( _verb1>1) {
      std::cerr << "COM: Set size for dataitem \"" 
		<< wa << '"' << " on pane " << pid 
    ...
    ex.msg = append_frame( ex.msg, COM_base::set_size);
    std::string s;
    s = s + "When processing dataitem " + wa;
    proc_exception( ex, s);
  }
...
void COM_base::set_object( const std::string &wa, const int pid,
			      void *obj_addr, void *casted_addr) throw(int) 
{
  COM_assertion_msg( obj_addr, "Caught NULL pointer");
  int size;
  get_dataitem( wa, NULL, NULL, &size, NULL);
...
// Associates an object with a specific window.
void COM_base::get_object( const std::string &wa, const int pid, 
			      void **ptr) throw(int) 
{
  int strd;
...
void COM_base::set_array( const std::string &wa, const int pid,
			     void *addr, int strd, int cap, 
			     bool is_const) throw(int) 
{
  try {
    if ( _verb1>1) {
      std::cerr << "COM: Set array for \"" << wa << "\" on pane " 
    ...
    ex.msg = append_frame( ex.msg, COM_base::set_array);
    std::string s;
    s = s + "When processing dataitem " + wa;
    proc_exception( ex, s);
  }
...
void COM_base::set_bounds( const std::string &wa, const int pane_id,
			      const void *lbnd, const void *ubnd) throw(int) 
{}

void COM_base::allocate_array( const std::string &wa, const int pid, 
				  void **addr, int strd, int cap) throw(int) 
{
  try {
    if ( _verb1>1) {
      std::cerr << "COM: Allocate array for \"" << wa << "\" on pane " 
		<< pid << " with stride ";
      if ( strd) std::cerr << strd;
      else std::cerr << "==#components";
    ...
    ex.msg = append_frame( ex.msg, COM_base::allocate_array);
    std::string s;
    s = s + "When processing dataitem " + wa;
    proc_exception( ex, s);
  }
...
void COM_base::resize_array( const std::string &wa, const int pid, 
				void **addr, int strd, int cap) throw(int) {
  try {
    if ( _verb1>1) {
      std::cerr << "COM: Resize array for \"" << wa << "\" on pane " 
		<< pid << " with stride ";
    ...
    ex.msg = append_frame( ex.msg, COM_base::resize_array);
    std::string s;
    s = s + "When processing dataitem " + wa;
    proc_exception( ex, s);
  }
...
void COM_base::append_array( const std::string &wa, const int pid, 
				const void *val, int v_strd, int v_size) throw(int)
{
  try {
    if ( _verb1>1)
      std::cerr << "COM: Appending array " << val << " for \"" << wa 
    ...
    ex.msg = append_frame( ex.msg, COM_base::append_array);
    std::string s("When processing dataitem "); s.append(wa);
    proc_exception( ex, s);
  }
}

void COM_base::
use_dataitem( const std::string &waname, const std::string &pwaname, 
	       int withghost,const char *cndname, int val) throw(int)
{
  try {
    if ( _verb1>1) {
      std::cerr << "COM: Using dataitem \"" << pwaname 
    ...
    DataItem *cnd=caaname.empty()?NULL:get_window(cawname).dataitem(caaname);
    if ( !caaname.empty() && cnd==NULL)
      throw COM_exception( COM_ERR_DATAITEM_NOTEXIST, cndname);

    get_window(awname).inherit( get_window( pawname).dataitem(paaname), 
				aaname, Pane::INHERIT_USE, withghost, cnd, val);
    _errorcode = 0;
  }
  catch ( COM_exception ex) {
    ex.msg = append_frame( ex.msg, COM_base::use_dataitem);
    std::string s;
    s = s + "When window " + waname + " uses dataitem " + pwaname;
    proc_exception( ex, s);
  }
}

void COM_base::
clone_dataitem( const std::string &waname, const std::string &pwaname,
		 int withghost, const char *cndname, int val) throw(int)
{
  try {
    if ( _verb1>1) {
      std::cerr << "COM: Cloning dataitem \"" << pwaname 
		<< "\" onto \"" << waname << '"' << std::endl;
    }
    std::string pawname, paaname;
    split_name( pwaname, pawname, paaname);
    ...
    ex.msg = append_frame( ex.msg, COM_base::clone_dataitem);
    std::string s;
    s = s + "When window " + waname + " clones dataitem " + pwaname;
    proc_exception( ex, s);
  }
}

void COM_base::
copy_dataitem( const std::string &waname, const std::string &pwaname,
		int withghost, const char *cndname, int val) throw(int)
{
  try {
    if ( _verb1>1) {
      std::cerr << "COM: Copying dataitem \"" << pwaname 
		<< "\" onto \"" << waname << '"' << std::endl;
    }
    std::string pawname, paaname;
    split_name( pwaname, pawname, paaname);
    ...
    ex.msg = append_frame( ex.msg, COM_base::copy_dataitem);
    std::string s;
    s = s + "When window " + waname + " copies dataitem " + pwaname;
    proc_exception( ex, s);
  }
}

void COM_base::
copy_dataitem( int trg_hdl, int src_hdl,
		int withghost, int ptn_hdl, int val) throw(int)
{
  try {
    if ( _verb1>1) {
      std::cerr << "COM: Copying dataitem with handle \"" << src_hdl 
		<< "\" onto \"" << trg_hdl << '"' << std::endl;
    }

    if ( trg_hdl <= 0 || src_hdl <= 0)
      throw COM_exception( COM_ERR_INVALID_DATAITEM_HANDLE);

    if ( _attr_map.is_immutable( trg_hdl))
      throw COM_exception(COM_ERR_IMMUTABLE);

    DataItem *trg = &get_dataitem(trg_hdl);
    trg->window()->inherit( &get_dataitem(src_hdl), trg->name(),
			    Pane::INHERIT_COPY, withghost, 
			    ptn_hdl?&get_dataitem(ptn_hdl):NULL, val);
    _errorcode = 0;
  }
  catch ( COM_exception ex) {
    ex.msg = append_frame( ex.msg, COM_base::copy_dataitem);
    ...
    sout << "When copying dataitem " << src_hdl << " onto " 
	 << trg_hdl << " with pattern dataitem " << ptn_hdl;
    proc_exception( ex, sout.str());
  }
}


void COM_base::deallocate_array( const std::string &wa, 
				    const int pid) throw(int) 
{
  try {
    if ( _verb1>1)
      std::cerr << "COM: Deallocate array for \"" << wa << "\" on pane " 
		<< pid << " to" << std::endl;
    ...
    ex.msg = append_frame( ex.msg, COM_base::deallocate_array);
    std::string s;
    s = s + "When processing dataitem " + wa;
    proc_exception( ex, s);
  }
...
// Register a new dataitem with given name
void COM_base::
get_dataitem( const std::string &wa, char *loc,
	       int *type, int *size, std::string *unit) throw(int) 
{
  try {
    if ( _verb1>1) // Print debugging info
      std::cerr << "COM: get dataitem \"" << wa << "\"" << std::endl;

    // Invoke Window::get_dataitem.
    std::string wname, aname;
    split_name( wa, wname, aname);

    get_window( wname).get_dataitem( aname, loc, type, size, unit);
    _errorcode = 0;
    ...
    ex.msg = append_frame( ex.msg, COM_base::get_dataitem);
    std::string s;
    s = s + "When processing dataitem " + wa;
    proc_exception( ex, s);
  }
...
void COM_base::get_size( const std::string &wa, int pid,
			    int *nitems, int *ng) throw(int) 
{
  try {
    if ( _verb1>1) {
      std::cerr << "COM: Get size for dataitem \"" 
    ...
    ex.msg = append_frame( ex.msg, COM_base::get_size);
    std::string s;
    s = s + "When processing dataitem " + wa;
    proc_exception( ex, s);
  }
...
void COM_base::get_array( const std::string &wa, const int pane_id,
			     void **addr, int *strd, int *cap, 
			     bool is_const) throw(int) 
{
...
void COM_base::get_array( const std::string &wa, const int pid,
			     Pointer_descriptor &addr, 
			     int *strd, int *cap, bool is_const) throw(int) {
  try {
    if ( _verb1>1) {
      std::cerr << "COM: Get array for dataitem \"" 
    ...
    ex.msg = append_frame( ex.msg, COM_base::get_array);
    std::string s;
    s = s + "When processing dataitem " + wa;
    proc_exception( ex, s);
  }
...
// Get the status of an dataitem.
int COM_base::get_status( const std::string &wa, int pid) throw(int) 
{
  try {
    if ( _verb1>1) {
      std::cerr << "COM: Get status for dataitem \"" 
		<< wa << '"' << " on pane " << pid << std::endl;
    }
    ...
    ex.msg = append_frame( ex.msg, COM_base::get_status);
    std::string s;
    s = s + "When processing dataitem " + wa;
    proc_exception( ex, s);
  }
...
void COM_base::copy_array( const std::string &wa, const int pid,
			      void *val, int v_strd, int v_size, 
			      int offset) throw(int) 
{
  try {
    if ( _verb1>1) {
      std::cerr << "COM: Copy array for dataitem \"" 
		<< wa << '"' << " on pane " << pid << std::endl;
    }
    ...
    ex.msg = append_frame( ex.msg, COM_base::copy_array);
    std::string s;
    s = s + "When processing dataitem " + wa;
    proc_exception( ex, s);
  }
...
void COM_base::get_bounds( const std::string &wa, const int pane_id,
			      void *lbnd, void *ubnd) throw(int) 
{}

int COM_base::check_bounds( const std::string &wa, 
			       int pane_id) throw(int) 
{ return 0; }

std::pair<int,int> // The pair of offset1 and offset2
COM_base::get_f90pntoffsets( const DataItem *a) {
  
  int n = DataItem::get_sizeof( COM_F90POINTER, a->size_of_components());
...
set_f90pointer( const std::string &waname, void *ptr, 
  ...
  try {
    if ( _verb1>1) 
      std::cerr << "COM: set pointer \"" << waname 
		<< " to " << ptr << std::endl;

    std::string wname, aname;
    split_name( waname, wname, aname);

    DataItem *a = get_window( wname).dataitem( aname);
    if ( a->location() != 'w')
      throw COM_exception( COM_ERR_NOT_A_WINDOW_DATAITEM,
			   append_frame(waname, COM_base::set_f90pointer));
	...
	*(void**)((char*)a->pointer()+offs.first)=NULL;
    ...
    ex.msg = append_frame( ex.msg, COM_base::set_f90pointer);
    proc_exception( ex, "");
  }
}
...
get_f90pointer( const std::string &waname, void *ptr, 
  ...
  try {
    if ( _verb1>1) 
      std::cerr << "COM: get pointer \"" << waname 
		<< " into " << ptr << std::endl;
    ...
    if ( a->location() != 'w')
      throw COM_exception( COM_ERR_NOT_A_WINDOW_DATAITEM,
			   append_frame(waname, COM_base::get_f90pointer));
    if ( a->data_type() == COM_F90POINTER) {
      std::pair<int,int> offs = get_f90pntoffsets( a);
    ...
    ex.msg = append_frame( ex.msg, COM_base::get_f90pointer);
    proc_exception( ex, "");
  }
}
...
get_communicator( const std::string &wname) throw(int) {
  try {
    if ( _verb1>1) 
      std::cerr << "COM: get the communicator of window \"" 
		<< wname << std::endl;
    if ( COMMPI_Initialized()) {
    ...
    ex.msg = append_frame( ex.msg, COM_base::get_window_npanes);
    std::string s;
...
get_panes( const std::string &wname, std::vector<int> &paneids_vec, 
  ...
  try {
    if ( _verb1>1) {
      std::cerr << "COM: get pane ids of window \"" 
		<< wname << "\" on ";
    ...
    ex.msg = append_frame( ex.msg, COM_base::get_panes);
    std::string s;
    s = s + "When processing window " + wname;
    proc_exception( ex, s);
  }
...
get_windows(std::vector<std::string> &names) throw(int){
  try {
    ...
    ex.msg = append_frame( ex.msg, COM_base::get_windows);
    std::string s;
...
get_modules(std::vector<std::string> &names) throw(int){
  try {
    ...
    ex.msg = append_frame( ex.msg, COM_base::get_modules);
    std::string s;
...
void COM_base::get_dataitems( const std::string &wname, int *natts, 
				  std::string &str, char **names) throw(int)
{
  try {
    if ( _verb1>1) 
      std::cerr << "COM: get dataitems of window \"" 
		<< wname << '"' << std::endl;

    std::vector<DataItem*> as;
    get_window(wname).dataitems( as);
    ...
    if ( _verb1>1) 
      std::cerr << "COM: Got dataitem names: " << str << std::endl;

    _errorcode = 0;
  }
  catch ( COM_exception ex) {
    ex.msg = append_frame( ex.msg, COM_base::get_dataitems);
    std::string s;
    s = s + "When processing window " + wname;
    proc_exception( ex, s);
  }
		    ...
		    std::string &str, char **names) throw(int)
{
  try {
    if ( _verb1>1) 
      std::cerr << "COM: get connectivities of window \"" << wname 
    ...
    if ( _verb1>1) 
      std::cerr << "COM: Got connectivity names: " << str << std::endl;

    _errorcode = 0;
  }
  catch ( COM_exception ex) {
    ex.msg = append_frame( ex.msg, COM_base::get_connectivities);
    std::string s;
    s = s + "When processing window " + wname;
    proc_exception( ex, s);
  }
	    ...
	    std::string &str, char **name) throw(int)
{
  try {
    if ( _verb1>1) 
      std::cerr << "COM: get parent of dataitem \"" << waname 
		<< "\" on pane " << pane_id << std::endl;
    ...
    if ( _verb1>1) 
      std::cerr << "COM: Got parent name: " << str << std::endl;

    _errorcode = 0;
  }
  catch ( COM_exception ex) {
    ex.msg = append_frame( ex.msg, COM_base::get_parent);
    std::string s;
    s = s + "When processing dataitem " + waname;
    proc_exception( ex, s);
  }
}


void COM_base::free_buffer( char **buf) 
{ if ( buf) delete [] *buf; *buf = NULL; }

void COM_base::free_buffer( int **buf) 
{ if ( buf) delete [] *buf; *buf = NULL; }
...
get_window_handle( const std::string &wname) throw(int) { 
  ...
  try {
    if ( _verb1>1) 
      std::cerr << "COM: get handle of window \"" << wname << "\": ";
    ...
    ex.msg = append_frame( ex.msg, COM_base::get_window_handle);
    proc_exception( ex, "");
  }
  return n;
}
...
get_window_object( int hdl) throw(int)
{
  if ( hdl <= 0) 
    throw COM_exception( COM_ERR_INVALID_WINDOW_HANDLE,
			 append_frame("", COM_base::get_window));
  return _window_map[hdl-1];
}

const Window * COM_base::
get_window_object( int hdl) const throw(int)
{
  if ( hdl <= 0) 
    throw COM_exception( COM_ERR_INVALID_WINDOW_HANDLE,
			 append_frame("", COM_base::get_window_object));
  return _window_map[hdl-1];
}

int COM_base::
get_dataitem_handle_const( const std::string &waname) throw(int) {
  int n(-1);
  try {
    if ( _verb1>1) 
      std::cerr << "COM: get const handle of dataitem \"" 
		<< waname << "\": ";
    ...
    ex.msg = append_frame( ex.msg, COM_base::get_dataitem_handle_const);
    proc_exception( ex, "");
  }
  return n;
}
...
int COM_base::
get_dataitem_handle( const std::string &waname) throw(int) {
  int n(-1);
  try {
    if ( _verb1>1) 
      std::cerr << "COM: get handle of dataitem \"" << waname << "\": ";
    std::string wname, aname;
    split_name( waname, wname, aname);
    ...
    ex.msg = append_frame( ex.msg, COM_base::get_dataitem_handle);
    proc_exception( ex, "");
  }
  return n;
}
...
get_function_handle( const std::string &wfname) throw(int) {
  ...
  try {
    if ( _verb1>1) 
      std::cerr << "COM: get handle of function \"" << wfname << "\": ";
    ...
    ex.msg = append_frame( ex.msg, COM_base::get_function_handle);
    proc_exception( ex, "");
  }
  return n;
}

void COM_base::set_function( const std::string &wfname, 
				Func_ptr ptr,
  ...
  try { 
    if ( _verb1>1) {
      int n=intents.size();
      std::cerr << "COM: init function \"" << wfname << '"'
    ...
    if ( intents.size() > Function::MAX_NUMARG) 
      throw COM_exception( COM_ERR_TOO_MANY_ARGS,
			   append_frame(wfname, COM_base::set_function));
    ...
    ex.msg = append_frame( ex.msg, COM_base::set_function);
    std::string s;
    s = s + "When processing dataitem " + wfname;
    proc_exception( ex, s);
  }
}
...
set_member_function_helper( const std::string &wfname, 
  ...
  try {
    ...
    if ( intents.size() > Function::MAX_NUMARG+1) 
      throw COM_exception( COM_ERR_TOO_MANY_ARGS,
			   append_frame(wfname, COM_base::set_member_function));
    ...
    DataItem *a = get_window( wname1).dataitem( aname1);
...
void COM_base::set_member_function( const std::string &wfname, 
				       Func_ptr ptr,
				       const std::string &waname,
				       const std::string &intents,
				       const COM_Type *types, 
...
get_num_arguments( const std::string &wf) throw(COM_exception) {
  ...
  if ( func==NULL) 
    throw COM_exception( COM_ERR_FUNCTION_NOTEXIST,
			 append_frame(wf, COM_base::set_member_function));
...
call_function( int wf, int count, 
  ...
  // COM prints out the trace upto (verb-1)/2 depth.
  ...
  try {
    ...
    const DataItem *attr = func->dataitem();
	...
	if ( h == 0 && func->intent(i)<='Z') { 
	  // Optional dataitem received a 0 dataitem handle
	  ps[i] = NULL;
	  if ( verb>1)
	    std::cerr << "ZERO DATAITEM HANDLE";
	  continue;
	}
	...
	(attr = func->dataitem())->data_type()==COM_F90POINTER) {
      COM_assertion_msg( _f90ptr_treat>=0, "No F90 pointer initalized");
...
//RAF      if (comm!=MPI_COMM_NULL) MPI_Barrier( comm);
    ...
    if ( verb) {
      std::cerr << "COM: DONE(" << _depth << ") " << std::endl;
    }
    ...
    ex.msg = append_frame( ex.msg, COM_base::call_function);
    char buf[10];
...
set_profiling( int i) {
  if ( _verb1>1) 
    std::cerr << "COM: init profiling level to " << i << std::endl; 
  _profile_on = i; 
...
set_profiling_barrier( int hdl, MPI_Comm comm) {
  ...
  try {
    ...
    if ( _verb1>1) 
      std::cerr << "COM: init profiling barrier for function handle " << hdl
    ...
    ex.msg = append_frame( ex.msg, COM_base::set_profiling_barrier);
    char buf[100];
...
print_profile( const std::string &fname, const std::string &header) {
  ...
  if ( _verb1>1)
     std::cerr << "COM: Appending profile into file \"" << fname
               << '"' << std::endl;
  ...
  if ( fname.size() == 0)
    ...
    if ( of == NULL) {
      std::cerr << "COM: Could not open file \"" << fname
		<< "\"\nCOM: Giving up profiling" << std::endl;
  ...
  if ( header.size() == 0)
    std::fputs( "************************COM simple profiling tool\
************************", of);
  else
...
int COM_base::get_sizeof( COM_Type type, int count) 
{ return DataItem::get_sizeof( type, count); }
...
get_window( const std::string &wname) throw(COM_exception)  
{
  ...
  if ( w ==NULL) 
    throw COM_exception( COM_ERR_WINDOW_NOTEXIST,
			 append_frame(wname, COM_base::get_window));
  return **w;
}

DataItem &COM_base::
get_dataitem( const int handle) throw(COM_exception) {
  DataItem *attr=NULL;
  ...
  if (attr==NULL) {
    ...
    throw COM_exception( COM_ERR_INVALID_DATAITEM_HANDLE,
			 append_frame(buf, COM_base::get_dataitem));
  }
  return *attr; 
}

const DataItem &COM_base::
get_dataitem( const int handle) const throw(COM_exception) { 
  const DataItem *attr=NULL;
...
get_function( const int handle) throw(COM_exception) { 
  ...
  if (func==NULL) {
    ...
    std::sprintf( buf, "%d", handle);
    throw COM_exception( COM_ERR_INVALID_FUNCTION_HANDLE,
			 append_frame(buf, COM_base::get_function));
  }
  return *func; 
}

const Function &COM_base::
...
int COM_base::split_name( const std::string &wa, std::string &wname, 
  ...
  if ( wname.empty() || aname.empty()) 
    if ( tothrow)
      throw COM_exception(COM_ERR_INVALID_DATAITEM_NAME,
			  append_frame(wa,COM_base::split_name));
    else
      return 1;
