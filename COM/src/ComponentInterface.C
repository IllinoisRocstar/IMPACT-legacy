...
/** \file ComponentInterface.C
 *  This file contains the implementation of the ComponentInterface object.
 *  @see com_devel.h, COM_base.C
 */
...
void ComponentInterface::alloc_array(const std::string &aname, const int pane_id,
			 void **addr, int strd, int cap) throw(COM_exception)
{
  if ( Connectivity::is_element_name( aname)) {
    ...
    if ( con==NULL) 
      throw COM_exception( COM_ERR_DATAITEM_NOTEXIST, append_frame
			   (name()+"."+aname,ComponentInterface::alloc_array));
    ...
    if ( a==NULL) 
      throw COM_exception( COM_ERR_DATAITEM_NOTEXIST, append_frame
			   (name()+"."+aname,ComponentInterface::alloc_array));
    reinit_dataitem( a, Pane::OP_ALLOC, addr, strd, cap);
  }
}

void ComponentInterface::resize_array(const std::string &aname, const int pane_id,
			  void **addr, int strd, int cap) throw(COM_exception)
{
  if ( Connectivity::is_element_name( aname)) {
    Pane &pn = pane(pane_id,true);
    ...
    if ( con==NULL) 
      throw COM_exception( COM_ERR_DATAITEM_NOTEXIST, append_frame
			   (name()+"."+aname,ComponentInterface::resize_array));
...
int ComponentInterface::owner_rank( const int pane_id) const {
...
Pane &ComponentInterface::pane( const int pid, bool insert) throw( COM_exception)
{
...
// Obtain the panes of the CI window on a give process.
void ComponentInterface::panes( std::vector<int> &pane_ids, int rank) 
{
...
// Obtain all the panes of the CI window.
void ComponentInterface::panes( std::vector<Pane*> &ps) 
{
