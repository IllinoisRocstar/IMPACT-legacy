...
/** \file DataItem.C
 *  Contains the implementation of the DataItem object.
 *  @see DataItem.hpp
 */
...
const char    *DataItem::_keywords[COM_NUM_KEYWORDS] = 
  { "nc", "1-nc", "2-nc", "3-nc", "conn", "mesh", 
    "pconn", "ridges", "1-ridges", "2-ridges", "pmesh", "data", "all" };

/// The locations of keywords.
const char     DataItem::_keylocs[] =  
  { 'n', 'n', 'n', 'n', 'e', 'p', 'p', 'p', 'p', 'p', 'p', 'p', 'p'};

/// The datatypes of keywords.
const COM_Type DataItem::_keytypes[] = 
...
Window       *DataItem::window()        { return _pane->window(); }
const Window *DataItem::window()  const { return _pane->window(); }

std::string DataItem::fullname() const 
{ return window()->name()+"."+name(); }

const void *DataItem::get_addr( int i, int j) const throw(COM_exception) {
  ...
  if ( i<0 || i>=size_of_items()) 
    throw COM_exception( COM_ERR_INDEX_OUT_OF_BOUNDS, append_frame
			 ( fullname(), DataItem::get_addr));
  return  ((char*)_ptr)+i*_nbytes_strd;
}

bool DataItem::size_set() const 
{ 
  if ( _loc == 'n' && _id != COM_NC) 
    return _pane->dataitem(COM_NC)->size_set();
  else if ( _loc == 'e' && _id != COM_CONN)
    return _pane->dataitem(COM_CONN)->size_set();
  else if ( _parent)
...
int DataItem::size_of_items() const 
{ 
  if ( _pane->ignore_ghost())
    return size_of_real_items();
  else if ( _loc == 'n' && _id != COM_NC) 
    return _pane->dataitem(COM_NC)->size_of_items();
  else if ( _loc == 'e' && _id != COM_CONN)
    return _pane->dataitem(COM_CONN)->size_of_items();
  else if ( _parent) 
...
int DataItem::maxsize_of_items() const 
{ 
  if (_pane->ignore_ghost()) 
    return maxsize_of_real_items();
  if ( _loc == 'n' && _id != COM_NC) 
    return _pane->dataitem(COM_NC)->maxsize_of_items();
  else if ( _loc == 'e' && _id != COM_CONN)
    return _pane->dataitem(COM_CONN)->maxsize_of_items();
  else if ( _parent)
...
int DataItem::size_of_ghost_items() const 
{ 
  if ( _pane->ignore_ghost()) 
    return 0;
  else if ( _loc == 'n' && _id != COM_NC) 
    return _pane->dataitem(COM_NC)->size_of_ghost_items();
  else if ( _loc == 'e' && _id != COM_CONN)
    return _pane->dataitem(COM_CONN)->size_of_ghost_items();
  else if ( _parent)
...
int DataItem::maxsize_of_ghost_items() const 
{ 
  if ( _pane->ignore_ghost()) 
    return 0;
  else if ( _loc == 'n' && _id != COM_NC) 
    return _pane->dataitem(COM_NC)->maxsize_of_ghost_items();
  else if ( _loc == 'e' && _id != COM_CONN)
    return _pane->dataitem(COM_CONN)->maxsize_of_ghost_items();
  else if ( _parent)
...
int DataItem::size_of_real_items() const 
{ 
  if ( _loc == 'n' && _id != COM_NC) 
    return _pane->dataitem(COM_NC)->size_of_real_items();
  else if ( _loc == 'e' && _id != COM_CONN)
    return _pane->dataitem(COM_CONN)->size_of_real_items();
  else if ( _parent)
...
int DataItem::maxsize_of_real_items() const
{ 
  if ( _loc == 'n' && _id != COM_NC) 
    return _pane->dataitem(COM_NC)->maxsize_of_real_items();
  else if ( _loc == 'e' && _id != COM_CONN)
    return _pane->dataitem(COM_CONN)->maxsize_of_real_items();
  else if ( _parent)
...
void DataItem::set_size( int nitems, int ngitems) throw(COM_exception)
{
  if ( _parent)
    throw COM_exception( COM_ERR_CHANGE_INHERITED,
			 append_frame( fullname(), DataItem::set_size));
...
void DataItem::set_pointer( void *p, int strd, int cap, 
			     int offset, bool is_const) throw(COM_exception)
{
  ...
  // 			 append_frame( fullname(), DataItem::set_pointer));
  ...
  if (direction==COPY_IN) {
    if ( is_const())
      throw COM_exception( COM_ERR_DATAITEM_CONST,
			   append_frame( fullname(), DataItem::copy_array));
...
append_array( const void *from, int strd, int nitem) throw(COM_exception)
{
  if ( !is_panel() && !is_windowed() || size_of_ghost_items())
    throw COM_exception( COM_ERR_APPEND_ARRAY,
			 append_frame(fullname(), DataItem::append_array));
...
void *DataItem::allocate( int strd, int cap, bool force) throw( COM_exception)
{
  int nitems=size_of_items(), ncomp=size_of_components();
  
  if ( strd!=1 && strd<ncomp) 
    throw COM_exception( COM_ERR_INVALID_STRIDE,
			 append_frame( fullname(), DataItem::allocate));
  if ( cap<nitems) 
    throw COM_exception( COM_ERR_INVALID_CAPACITY,
			 append_frame( fullname(), DataItem::allocate));
  ...
  try {
    int type = data_type();
    // Go ahead to allocate for the dataitem if it was not initialized 
  ...
  CATCHEXP_APPEND(DataItem::allocate) CATCHBADALLOC_APPEND(DataItem::allocate);
  return _ptr;
}

void DataItem::
inherit( DataItem *parent, bool clone, bool withghost, int depth) throw(COM_exception) 
{
  DataItem *root = parent->root();
  _loc=root->_loc; _ncomp=root->_ncomp; _type=root->_type; 
  ...
  if ( !clone) {
    ...
    } CATCHEXP_APPEND(DataItem::inherit);
  }
  else {
    _parent = NULL;
    // Set size
    ...
    }  CATCHEXP_APPEND(DataItem::inherit) CATCHBADALLOC_APPEND(DataItem::allocate);
...
int DataItem::deallocate() throw( COM_exception) {
  try {
    // Deallocate dataitem and set individual components to not initialized.
  ...
  CATCHEXP_APPEND(DataItem::deallocate);
  return 0;
}

int DataItem::get_sizeof( COM_Type type, int count) {
  switch (type) {
  case COM_CHAR:
...
bool DataItem::compatible_types( COM_Type t1, COM_Type t2) {
  if (t1 == t2) return true;
  if ( DataItem::get_sizeof( t1, 1) != DataItem::get_sizeof( t2, 1)) 
    return false;
