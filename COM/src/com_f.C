 ...
 *  @see utilities.f90, comf90.h, mapptr.h
 */
 ...
 *  @see comf90.h, utilities.f90, mapptr.h
 */
...
{ COM_base::set_default_communicator( COMMPI_Comm_f2c(comm, MPI_Comm())); }

extern "C" int COM_F_FUNC2(com_get_default_communicator, 
			   COM_GET_DEFAULT_COMMUNICATOR)()
{ return COMMPI_Comm_c2f( COM_base::get_default_communicator()); }
...
extern "C" void COM_F_FUNC2(com_load_module, COM_LOAD_MODULE)
  ( const char *libname, const char *winname, 
    int l1, int l2) 
{
  COM_get_com()->load_module( std::string( libname, l1),
...
{
  COM_get_com()->unload_module( std::string( libname, l1), "");
}
...
class COM_base_friend : public COM_base {
public:
  using COM_base::_f90ptr_treat;
};
...
inline int get_f90ptr_treat() {
  COM_base_friend *rcom = (COM_base_friend *)COM_get_com();
...
{ CHKLEN(len); COM_get_com()->delete_window( string( str, len)); }

extern "C" void COM_F_FUNC2(com_window_init_done_1arg, COM_WINDOW_INIT_DONE_1ARG)
  ( const char *w_str, int w_len) 
{ CHKLEN(w_len); COM_get_com()->window_init_done( string(w_str,w_len)); }

extern "C" void COM_F_FUNC2(com_window_init_done_2arg, COM_WINDOW_INIT_DONE_2ARG)
  ( const char *w_str, int *c, int w_len) 
{ CHKLEN(w_len); COM_get_com()->window_init_done( string(w_str,w_len), *c); }

extern "C" void COM_F_FUNC2(com_delete_pane,COM_DELETE_PANE)
  ( const char *w_str, const int &pid, int w_len) 
{ CHKLEN(w_len); COM_get_com()->delete_pane( string(w_str,w_len), pid); }

extern "C" void COM_F_FUNC2(com_new_dataitem,COM_NEW_DATAITEM)
    ...
    int wa_len, int l_len, int u_len) 
{
  ...
  COM_get_com()->new_dataitem( string( wa_str,(int)wa_len), loc, type, 
				   size, string( u_str,(int)u_len));
}

extern "C" void COM_F_FUNC2(com_delete_dataitem,COM_DELETE_DATAITEM)
  ( const char *wa_str, int wa_len) 
{
  CHKLEN(wa_len); 
  COM_get_com()->delete_dataitem( string( wa_str,wa_len));
}
...
{ 
  CHKLEN( len);
  COM_get_com()->set_size( std::string(wa_str,len), pane_id, size); 
}
...
{ 
  CHKLEN( len);
  COM_get_com()->set_size( std::string(wa_str,len), pane_id, size, ng); 
}

// Register an dataitem name.
...
{ COM_get_com()->set_array( std::string(wa_str,len), 
			       pane_id, NULL); }
...
template <int dim, COM_Type type>
void
com_set_address_f( COM_base::Pointer_descriptor &ptr, 
		   void *addr, int l=0) {
  Func_ptr f;
...
com_set_array_helper( const std::string &waname, int pane_id, void *addr, 
  ...
  COM_get_com()->set_array( waname, pane_id, ptr, strd, cap);
}
...
{ \
  CHKLEN(wa_len); \
  COM_get_com()->set_array( string(wa_str,wa_len), pid, addr); \
}
...
com_set_array_f( const char *waname, int pane_id, void *addr, 
  ...
  if ( f90ptr_treat == COM_base::FPTR_INSERT) {
    ...
    if ( f90ptr_treat == COM_base::FPTR_APPEND) {
      com_set_array_helper<dim,type>( string(waname, wa_len), pane_id, addr, 
				      strd, cap, l2);
    }
    else {
...
{ \
  CHKLEN(wa_len); \
  COM_get_com()->set_array( string(wa_str,wa_len), pid, addr, strd); \
}
...
{ \
  CHKLEN(wa_len); \
  COM_get_com()->set_array( string(wa_str,wa_len), pid, addr, strd, cap); \
}
...
com_set_array_const_helper(const std::string &waname, int pane_id, void *addr, 
  ...
  COM_get_com()->set_array( waname, pane_id, ptr, strd, cap, true);
}
...
{ \
  CHKLEN(wa_len); \
  COM_get_com()->set_array( string(wa_str,wa_len), pid, addr, true); \
}
...
{ \
  CHKLEN(wa_len); \
  COM_get_com()->set_array( string(wa_str,wa_len), pid, addr, strd, true); \
}
...
{ \
  CHKLEN(len);\
  COM_get_com()->set_bounds( string(wa_str,len), pane_id, lbound, ubound); \
}
...
{ COM_get_com()->allocate_array( std::string( wa_str, len)); }

extern "C" void COM_F_FUNC2( com_alloc_array_pane, COM_ALLOC_ARRAY_PANE)
  ( const char *wa_str, const int &pane_id, int len)
{ COM_get_com()->allocate_array( std::string( wa_str, len), pane_id); }
...
com_obtain_array_helper( const std::string &waname, int pane_id, void *addr, 
  ...
  COM_base::Pointer_descriptor ptr(NULL, dim);
  if ( mode==AM_ALLOC) {
    COM_get_com()->allocate_array( waname, pane_id, NULL, strd, cap);
    COM_get_com()->get_array( waname, pane_id, ptr);
  }
  else if ( mode==AM_RESIZE) {
    COM_get_com()->resize_array( waname, pane_id, NULL, strd, cap);
    COM_get_com()->get_array( waname, pane_id, ptr);
  }
    ...
    COM_get_com()->copy_array( waname, pane_id, ptr.ptr,
				  strd, cap, offset);
  }
  else {
    COM_get_com()->get_array( waname, pane_id, ptr, &strd, 
				 &cap, mode==AM_GETC);
  }
...
extern "C" void COM_F_FUNC2( com_resize_array_win, COM_RESIZE_ARRAY_WIN)
  ( const char *wa_str, int len)
{ COM_get_com()->resize_array( std::string( wa_str, len)); }

extern "C" void COM_F_FUNC2( com_resize_array_pane, COM_RESIZE_ARRAY_PANE)
  ( const char *wa_str, const int &pane_id, int len)
{ COM_get_com()->resize_array( std::string( wa_str, len), pane_id); }
...
{ \
  CHKLEN(wa_len); \
  COM_get_com()->append_array( string(waname,wa_len), pane_id, \
				  val, v_strd, v_size); \
}
...
com_append_array_helper( const std::string &waname, int pane_id, 
  ...
  COM_get_com()->append_array( waname, pane_id, v_ptr, v_strd, v_size);
}
...
{ \
  CHKLEN(len);\
  COM_get_com()->get_bounds( string(wa_str,len), pane_id, lbound, ubound); \
}
...
{ 
  CHKLEN(len);
  return COM_get_com()->check_bounds( string(wa_str,len), pane_id); 
}

extern "C" void COM_F_FUNC2(com_use_dataitem, COM_USE_DATAITEM)
  ( const char *wname, const char *attr, int w_len, int a_len) 
{
  CHKLEN(w_len); CHKLEN(a_len);
  COM_get_com()->
    use_dataitem( string(wname, w_len), string(attr, a_len));
}

extern "C" void COM_F_FUNC2(com_clone_dataitem,COM_CLONE_DATAITEM)
  ( const char *wname, const char *attr, int w_len, int a_len) 
{
  CHKLEN(w_len); CHKLEN(a_len);
  COM_get_com()->
    clone_dataitem( string(wname, w_len), string(attr, a_len));
}

extern "C" void COM_F_FUNC2(com_copy_dataitem,COM_COPY_DATAITEM)
  ( const char *wname, const char *attr, int w_len, int a_len) 
{
  CHKLEN(w_len); CHKLEN(a_len);
  COM_get_com()->
    copy_dataitem( string(wname, w_len), string(attr, a_len));
}

extern "C" void COM_F_FUNC2(com_copy_dataitem_hdls,COM_COPY_DATAITEM_HDLS)
  ( const int &trg_hdl, const int &src_hdl) 
{
  COM_get_com()->copy_dataitem( trg_hdl, src_hdl);
}

extern "C" void COM_F_FUNC2(com_use_dataitem_ghost, COM_USE_DATAITEM_GHOST)
  ( const char *wname, const char *attr, const int &with_ghost,
    int w_len, int a_len) 
{
  CHKLEN(w_len); CHKLEN(a_len);
  COM_get_com()->
    use_dataitem( string(wname, w_len), string(attr, a_len), with_ghost);
}

extern "C" void COM_F_FUNC2(com_clone_dataitem_ghost, COM_CLONE_DATAITEM_GHOST)
  ( const char *wname, const char *attr, const int &with_ghost,
    int w_len, int a_len) 
{
  CHKLEN(w_len); CHKLEN(a_len);
  COM_get_com()->
    clone_dataitem( string(wname, w_len), string(attr, a_len), with_ghost);
}

extern "C" void COM_F_FUNC2(com_copy_dataitem_ghost, COM_COPY_DATAITEM_GHOST)
  ( const char *wname, const char *attr, const int &with_ghost,
    int w_len, int a_len) 
{
  CHKLEN(w_len); CHKLEN(a_len);
  COM_get_com()->
    copy_dataitem( string(wname, w_len), string(attr, a_len), with_ghost);
}

extern "C" void COM_F_FUNC2(com_copy_dataitem_ghost_hdls,COM_COPY_DATAITEM_GHOST_HDLS)
  ( const int &trg_hdl, const int &src_hdl, const int &with_ghost ) 
{
  COM_get_com()->copy_dataitem( trg_hdl, src_hdl, with_ghost);
}

extern "C" void COM_F_FUNC2(com_use_dataitem_sub, COM_USE_DATAITEM_SUB)
  ( const char *wname, const char *attr, const int &with_ghost, 
...
extern "C" void COM_F_FUNC2(com_clone_dataitem_sub, COM_CLONE_DATAITEM_sub)
  ( const char *wname, const char *attr, const int &with_ghost,  
    const char *ptnname, const int &val,
    int w_len, int a_len, int p_len) 
{
...
extern "C" void COM_F_FUNC2(com_copy_dataitem_sub, COM_COPY_DATAITEM_sub)
  ( const char *wname, const char *attr, const int &with_ghost,  
    const char *ptnname, const int &val,
    int w_len, int a_len, int p_len) 
{
...
extern "C" void COM_F_FUNC2(com_copy_dataitem_ghost_sub_hdls,
			    COM_COPY_DATAITEM_GHOST_SUB_HDLS)
  ( const int &trg_hdl, const int &src_hdl, const int &with_ghost,
...
{
  CHKLEN(wa_len); 
  COM_get_com()->deallocate_array( string( wa_str, wa_len), 0);
}
...
{
  CHKLEN(wa_len); 
  COM_get_com()->deallocate_array(string(wa_str, wa_len), pid);
}

extern "C" void COM_F_FUNC2(com_get_size1, COM_GET_SIZE1)
  ( const char *wa_str, const int &pane_id, int *size, int len)
{ COM_get_com()->get_size( std::string(wa_str,len), pane_id, size); }

extern "C" void COM_F_FUNC2(com_get_size2, COM_GET_SIZE2)
  ( const char *wa_str, const int &pane_id, int *size, int *ng, int len)
{ COM_get_com()->get_size( std::string(wa_str,len), pane_id, size, ng); }

extern "C" void COM_F_FUNC2(com_get_dataitem,COM_GET_DATAITEM)
    ...
    char *u_str,  int wa_len, int l_len,  int u_len) 
{
  ...
  std::string unit;
  COM_get_com()->get_dataitem( string( wa_str,wa_len), loc, type, 
				   size, &unit);
    ...
    const COM_Type *types,  int wf_len,  int i_len) 
{
  ...
  COM_get_com()->set_function( string(wf_str,wf_len), func,
    ...
    const COM_Type *types,  int wf_len,  int a_len,  int i_len) 
{
  ...
  COM_get_com()->set_member_function( string(wf_str,wf_len), func,
					 string(wa_str,a_len),
...
{
  CHKLEN(w_len); 
  *comm = COMMPI_Comm_c2f(COM_get_com()->
...
{
  int f90ptr_treat = get_f90ptr_treat();
  int w_len=(f90ptr_treat==COM_base::FPTR_INSERT)?l2:l1; CHKLEN(w_len);

  std::vector<int> vec;
  int *pids;
  COM_get_com()->get_panes( string(wname, w_len), vec, rank, &pids);
  *npanes = vec.size();
  ...
  ( const char *wname, int *npanes,  int w_len) 
{
  CHKLEN(w_len);
  std::vector<int> vec;
  COM_get_com()->get_panes( string(wname, w_len), vec); 
  *npanes = vec.size();
}
...
extern "C" void COM_F_FUNC2(com_get_dataitems,COM_GET_DATAITEMS)
     ...
     int l1,  int l2) 
{
  ...
  std::string str;
  char *atts;
  COM_get_com()->get_dataitems( string(wname, w_len), na, str, &atts); 
     ...
     int l1,  int l2) 
{
  ...
  std::string str;
  char *conns;
  COM_get_com()->get_connectivities( string(wname, w_len), pane_id,
					nc, str, &conns); 
...
{ 
  ...
  std::string str;
  char *name;
  COM_get_com()->get_parent( string(waname, w_len), pane_id, str, &name); 
...
{ 
  CHKLEN(len); 
  return COM_get_com()->get_status( std::string(waname,len), pane_id); 
}
...
{
  CHKLEN(len); 
  return COM_get_com()->get_window_handle( std::string(wname,len)); 
}

extern "C" int COM_F_FUNC2(com_get_dataitem_handle, COM_GET_DATAITEM_HANDLE)
  ( const char *waname,  int len) 
{ 
  CHKLEN(len); 
  return COM_get_com()->get_dataitem_handle( std::string(waname,len)); 
}

extern "C" int COM_F_FUNC2( com_get_dataitem_handle_const, COM_GET_DATAITEM_HANDLE_CONST)
  ( const char *waname,  int len) {
 
  CHKLEN(len); 
  return COM_get_com()->get_dataitem_handle_const( std::string(waname,len)); 
}
...
{
  CHKLEN(len); 
  return COM_get_com()->get_function_handle( std::string(wfname,len)); 
}
...
extern "C" void COM_F_FUNC2(com_call_function, COM_CALL_FUNCTION)
  ...
  COM_get_com()->call_function(wf, argc, args, lens, false);
}
...
extern "C" void COM_F_FUNC2( com_icall_function, COM_ICALL_FUNCTION)
  ...
  COM_get_com()->icall_function(wf, argc, args, lens, false);
}
...
extern "C" void COM_F_FUNC2( com_set_pointer, COM_SET_POINTER)
  ...
  if ( get_f90ptr_treat() == COM_base::FPTR_INSERT) {
    ...
    COM_get_com()->set_f90pointer( std::string(waname,len), ptr, func, l1); 
  }
  else {
    int len=l1; CHKLEN( len);
    COM_get_com()->set_f90pointer( std::string(waname,len), ptr, func, l2);
  }
}
...
extern "C" void COM_F_FUNC2(com_get_pointer, COM_GET_POINTER)
  ...
  if ( f90ptr_treat == COM_base::FPTR_INSERT) {
    ...
    //       the pointer and l2 is the length of the character string.
    int len=l2; CHKLEN( len);
    COM_get_com()->get_f90pointer( std::string(waname,len), ptr, func, l1); 
  }
  else {
    int len=l1; CHKLEN( len);
    COM_get_com()->get_f90pointer( std::string(waname,len), ptr, func, l2);
  }
}
...
{
  COM_get_com()->set_profiling_barrier
...
extern "C" int COM_F_FUNC2( com_chkptr_c, COM_CHKPTR_C) 
  ...
  case 0: // Being called from COM_init
    insert_or_append = COM_base::FPTR_NONE;
    ptrinfo1 = ptrinfo2 = 0;
  ...
  case 1:
    if ( (int)len3 == 17 && (int)len4 == 33) 
      insert_or_append = COM_base::FPTR_INSERT;
    else {
      // std::cout << "len(1,2,3,4) = " << (int)len1 << "," 
    ...
    if ( insert_or_append == COM_base::FPTR_INSERT) {
  ...
  case 2: { // This stage, the arguments are switched
    int verb = COM_get_com()->get_verbose();
    if ( insert_or_append == COM_base::FPTR_INSERT) {
      COM_assertion_msg( ptrinfo1 == len2-((char*)ptr2-(char*)NULL) &&
			 ptrinfo2 == len1-((char*)ptr1-(char*)NULL),
			 "Incorrect handling of Fortran 90 pointers");
      if ( verb) 
	std::cerr << "COM: Setting f90 pointer treatment to INSERT" << std::endl;
    }
    else {
      if ( ptrinfo1 != 0 && ptrinfo1 == len4-((char*)ptr2-(char*)NULL) && 
	   ptrinfo2 != 0 && ptrinfo2 == len3-((char*)ptr1-(char*)NULL)) {
	if (  verb) 
	  std::cerr << "COM: Setting f90 pointer treatment to APPEND. \n";
	insert_or_append = COM_base::FPTR_APPEND;
      }
      else if ( verb) 
	std::cerr << "COM: Setting f90 pointer treatment to NONE.\n";
