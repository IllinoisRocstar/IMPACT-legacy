...
public:
        ...
        void SetParam(void *keyPtr,void *valuePtr)
                {
                        ...
                        if(packInput.IsSet(*keyString))
                                packInput.SetValue(*keyString,*valueString);
                        else{
        ...
        int GenerateSnap(int i,double x,double y,double z,
                         double theta,double aspectRatio,
                         ...
                         const std::string &pngString)
                {
                        ...
                        if(!errorStatus){
                                ...
                                if(!errorStatus){
                                        ...
                                        command.assign("cp " + sourceFileName + ".png " + pngString);
                                        errorStatus = system(command.c_str());
                                }
                        }
                        return(errorStatus);
        ...
        void GenerateViews(int *numAnglesIn,int *whichAngleIn,void *sourceFileName,void *viewFileName)
                {

                        int increments = *numAnglesIn;
                        //    int rdfstats = *doRDFIn;
                        int whichAngle = *whichAngleIn;
                        std::string *fileNamePtr = static_cast<std::string *>(viewFileName);
                        std::string *sourceNamePtr = static_cast<std::string *>(sourceFileName);
                        double xsize = 2.0/packInput.aspectRatio;
                        double zsize = 2.0/packInput.aspectRatio;
                        double ysize = 2.0;
                        double x1 = xsize + 2.0;
                        ...
                        if(increments > 0){
                                ...
                                for(int i=0; i < increments; i++){
                                        if(whichAngle < 0 || whichAngle == i){
                                                std::ifstream Inf;
                                                Inf.open(sourceNamePtr->c_str());
                                                if(Inf){
                                                        Inf.close();
                                                        std::ostringstream dirStr;
                                                        dirStr << "makeimage_" << (i < 10 ? "0" : "") << i;
                                                        ...
                                                        std::ostringstream imOut;
                                                        imOut << *fileNamePtr << "_"
                                                              << (i < 10 ? "0" : "") << i << ".png";
                                                        std::string pngString(imOut.str());
            
                                                        if(GenerateSnap(i,x1,y1,z1,theta,packInput.cylinderAspect,*sourceNamePtr,pngString)){
        ...
        void GenerateRDF(void *sourceFileName){

                int numberOfParticles = 0;
                std::string *sourceFileNamePtr = static_cast<std::string *>(sourceFileName);
                ...
                std::string command = RDFGENCOMMAND + std::string(" < ") + *sourceFileNamePtr
                        + std::string(" > ") + *sourceFileNamePtr + std::string("_rdf.out");
                system(command.c_str());
        }
private:
