...
void Rocin::read_parameter_file( const char* file_name,
  ...
  if(param_window){
	...
	COM_assertion_msg( param_att->location()=='w' &&
			   ...
			   "The dataitem must be a windowed scalar dataitem.");
	// Obtain address for the current dataitem.
	...
	case COM_DOUBLE: {
          // replace 'd' or 'D' by 'e'
          replace( param_list[i].second.begin(), param_list[i].second.end(),'d','e'); 
          replace( param_list[i].second.begin(), param_list[i].second.end(),'D','e');
