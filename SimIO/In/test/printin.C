...
void COM_print_window(const char *wName, std::ostream &ostr) {
  ...
  COM_get_dataitems( wName, &na, &atts);
  ...
  for ( int i=0; i<na; ++i) {
    ...
    COM_get_dataitem( (w+aname).c_str(), &loc, &type, &ncomp, &units);
    ostr << "" << aname << " ("<<units<<")  " << ncomp << " x " << COM_get_type(type) <<"   ";

    int nitems;
    if ( loc == 'w') { /* window dataitem */
      int ng;
      COM_get_size((w+aname).c_str(), 0, &nitems, &ng);
      ostr << "    Window: " << nitems << " items ("<<ng<<" ghosts)";
    }
    else if (loc=='p') { /* panel dataitem */
      for ( int i=0; i<np; ++i) {
	  // Obtain the size for a panel dataitem.
	  int ng;
	  COM_get_size((w+aname).c_str(), pane_ids[i], &nitems, &ng);
	  ostr << "\n    Panel("<<pane_ids[i]<<"): "<<nitems<<" items ("<<ng<<" ghosts)";
      }
    }
    else if (loc=='e') { /* element dataitem */
      nitems = nelems;
      ostr << "    Element";
    }
    else if (loc=='n') { /* node dataitem */
      nitems = nnodes;
      ostr << "    Node";
    } 
    else {
      ostr << "  (Unrecognized dataitem location "<<loc<<")";
    }
    ostr << std::endl;
