...
static void io_pane_dataitem(const char* fname, const COM::Pane* pane,
                              const COM::DataItem* attr, const char* timelevel,
                              const char* coordsys,
                              const std::string& errorhandle, const int mode);
                    ...
                    const std::string& errorhandle, const int mode)
{
  ...
  if (with_mesh) {
    ...
    io_pane_coordinates( fname, pane, timelevel, coordsys.c_str(), 
			 pane->dataitem(COM::COM_NC)->unit().c_str(),
                         errorhandle, mode);
    ...
    // Write out ridges
    io_pane_dataitem( fname, pane, pane->dataitem(COM::COM_RIDGES), 
		       timelevel, NULL, errorhandle, mode);
    if (attr->id() == COM::COM_MESH) return;

    // Write out pane connectivity
    io_pane_dataitem( fname, pane, pane->dataitem(COM::COM_PCONN), 
		       timelevel, NULL, errorhandle, mode);
  ...
  else if ( attr->id() == COM::COM_ALL || attr->id() == COM::COM_DATA) {
    std::vector<const DataItem*> attrs;
    pane->dataitems(attrs);
    std::vector<const DataItem*>::const_iterator it;
    for (it=attrs.begin(); it!=attrs.end(); ++it) {
      io_pane_dataitem(fname, pane, *it, timelevel, NULL, errorhandle, mode);
    }
  } else {
    // Call io_pane_dataitem on the dataitem in the given pane.
    io_pane_dataitem( fname, pane, pane->dataitem(attr->id()), 
		       timelevel, NULL, errorhandle, mode);
  }
}
...
#endif // DEBUG_DUMP_PREFIX
  int ncomp=pane->dataitem(COM::COM_NC)->size_of_components();
  for ( int i=COM::COM_NC1; i<COM::COM_NC1+ncomp; ++i) {
    io_pane_dataitem(fname, pane, pane->dataitem(i),
                              ...
                              const std::string& errorhandle, const int mode)
{
  ...
  if (attr->is_nodal()) {
    ...
    shape[1] = shape[2] = 1;
  } else { // skip windowed dataitems that are pointers
    return; 
  }
  ...
  if (mode >= 0 && (is_vector3 || is_tensor9)) {
    ...
    for (int i=0; i<ncomp; ++i) {
      const DataItem* pa = pane->dataitem(attr->id() + i + 1);
      if (pa->pointer()) 
  ...
  // Initialize unit and dataitem name
  ...
  // Append the name and location of the dataitem
  std::string a_name = attr->name();
  sout << a_name << '|' << attr->location();
  int li0 = sout.str().size();
  sout << ',' << ng2 << " AT TIME " << timelevel;

  DEBUG_MSG("Preparing to write dataitem '" << attr->name() << "' (ncomp == "
            << ncomp << ')');
  // Perform IO the data
  for (int i=0; i<ncomp; ++i) {
    int label_index = li0;
    const DataItem* pa = pane->dataitem(attr->id() + i + (ncomp>1));

    DEBUG_MSG("Preparing to write dataitem '" << pa->name() << '\'');
    // Set label
    std::string label = sout.str();
    ...
    else if ( ncomp>1) { // Use <d>-attr as the dataitem name.
      a_name = pa->name();
                        ...
                        const void* minv, const void* maxv)
{
  ...
  if (dim == 1) {
    ...
    if (stride > 1) {
      int s = COM::DataItem::get_sizeof(type, 1);
...
{ \
  ...
  if (status == FAIL && errorhandle != "ignore") { \
    std::cerr << "Rocout::write_dataitem: " #routine " (line " \
