  ...
  int read_mesh( const char *fname, const std::string &wname) {
	...
	get_nextline( is, buf);
      ...
      std::cout << "Reading " << nn << " nodes and " 
		<< ne << " faces for patch " << pid << std::endl;
...
private:
  ...
  void read_pane_elems( std::istream &is, const std::string &wname, 
	...
	if ( num_ele==ne) break;
	get_nextline(is,buf); numEdges2=0;
	std::istringstream sin(buf);
	sin >> numEdges2;
	for ( int i=0; i<numEdges2; ++i) sin >> vs[i];
      ...
      switch (numEdges1) {
      case 3: str= wname+".:t3:"+ext1 ;	break;
      case 6: str= wname+".:t6:"+ext1 ;	break;
      case 4: str= wname+".:q4:"+ext1 ;	break;
      case 8: str= wname+".:q8:"+ext1 ;	break;	
      case 9: str= wname+".:q9:"+ext1 ;	break;	
      default: ;
      }

      COM_set_size( str.c_str(), pane_id, conns[0].size());
      int *es;  COM_allocate_array( str.c_str(), pane_id, &(void*&)es);
