...
// 			 quad_mesh_fd[i], 4, 1);
  ...
  COM_new_dataitem("quad.disp", 'n', COM_DOUBLE, 3, "m");
  COM_new_dataitem("quad.press", 'e', COM_DOUBLE, 1, "Pascal");
  ...
  COM_new_window("tri");
  COM_new_dataitem("tri.disp", 'n', COM_DOUBLE, 3, "m");
  COM_new_dataitem("tri.press", 'e', COM_DOUBLE, 1, "Pascal");
...
// 			 tri_mesh_fd[i], 3, 1);
  ...
  int quad_mesh = COM_get_dataitem_handle( "quad.mesh");
  ...
  if ( comm_size == 1) {    
    ...
    COM_call_function( RFC_write, &tri_mesh, &quad_mesh, "tri", "quad", "CGNS");
  ...
  if ( argc > 7) {
    ...
    int tri_all = COM_get_dataitem_handle( "tri.all");
    int quad_press = COM_get_dataitem_handle( "quad.press");
    ...
    int tri_press = COM_get_dataitem_handle( "tri.press");
    COM_call_function( RFC_transfer, &tri_press, &quad_press);
    COM_call_function( RFC_transfer, &quad_press, &tri_press);

    int tri_disp = COM_get_dataitem_handle( "tri.disp");
    int quad_disp = COM_get_dataitem_handle( "quad.disp");
